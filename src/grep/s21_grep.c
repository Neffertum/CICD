...
int output(int argc, char **argv) {
  ...
  myflags.e = myflags.i = myflags.v = myflags.c = myflags.l = myflags.n =
      myflags.h = myflags.s = myflags.f = myflags.o = 0;
  FILE *file;
  char temp[256];
  int temp_place;
  int start = 0;
    ...
    for (int i = 1; i < argc; i++) {
      if (argv[i][0] == '-') {
        ...
        for (size_t r = 1; r < strlen(argv[i]); r++) {
          ...
          if (argv[i][r] != 'e' && argv[i][r] != 'i' && argv[i][r] != 'v' &&
              argv[i][r] != 'c' && argv[i][r] != 'l' && argv[i][r] != 'n' &&
              argv[i][r] != 'h' && argv[i][r] != 's' && argv[i][r] != 'f' &&
              argv[i][r] != 'o') {
            have_flag = -1;
            break;
          }
...
int make_magic(struct flags myflags, FILE *file, char *temp, char *filename,
  ...
  if (err == 0) {
    while (getline(&str, &t_size, file) != -1) {
      if (myflags.o == 1 && myflags.v != 1 && myflags.c != 1 &&
          myflags.l != 1) {
